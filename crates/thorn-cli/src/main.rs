...
fn main() {
    ...
    // Ask each plugin for its CLI params and add them to the command
    let plugins_tmp = plugins::create_all();
    // (plugin_name, Vec<(param_name, takes_value)>)
    ...
    // Build the linter with the plugin-provided graph
    let mut linter = thorn_core::Linter::new(graph);
    for plugin in plugins {
        linter.register(plugin);
    }

    // Collect excludes: CLI + config + plugin-specific
